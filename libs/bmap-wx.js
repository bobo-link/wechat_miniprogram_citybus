...
class BMapWX {
    ...
    reverse_geocoding(param){
      ...
      let otherparam = {
        success: param["success"] || function () { },
        fail: param["fail"] || function () { },
        locationcomplete: param["complete"] || function(){ } 
      };
      //微信获取地址
      let locationsuccess = function (result) {
        rever_param['location'] = result["latitude"] + ',' + result["longitude"];
        wx.request({
          url: 'https://api.map.baidu.com/reverse_geocoding/v3/',
          data: rever_param,
          header: {
          ...
          success(res) {
            ...
            if (data["status"] === 0 ) {
              //从接口返回的数据获取adcode
              let adcode = data.result.addressComponent.adcode
              otherparam.success({
                'adcode':adcode,
                'origin_data':data
              });
